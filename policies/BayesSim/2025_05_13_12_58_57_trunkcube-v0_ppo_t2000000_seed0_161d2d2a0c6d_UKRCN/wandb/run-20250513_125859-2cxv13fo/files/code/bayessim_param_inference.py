...
def main():
        ...
        if args.env == "trunkcube-v0":
                ...
                env = gym.make(args.env, config_ext_dict=ext_dict,)
        ...
        if not args.training_only:
                ...
                gen_posterior, expert_posterior = get_results_from_true_obs(env_params=list(env.dyn_ind_to_name.values()),
                        true_obs=true_obs, generator=g, inf=inf_mdn, shapes=shapes,
                        p_lower=g.env.min_task, p_upper=g.env.max_task, env_name=env.config['scene'], save_path=run_path,wandb=wandb)
                ...
                if args.true_dist:
                        ...
                        dist_posterior = get_results_from_current_dist(env_params=list(env.dyn_ind_to_name.values()),
                                true_obs=true_obs, generator=g, inf=inf_mdn, shapes=shapes,
