...
def main():
    ...
    if args.reduced_sensing and args.env != "trunkcube-v0":
        raise ValueError(f"Reduced sensing setting implemented only for trunkcube-v0")
    ...
    if args.env == "trunkcube-v0":
        env = make_vec_env(args.env, n_envs=args.now, seed=args.seed, vec_env_cls=SubprocVecEnv, env_kwargs={'reduced_sensing': args.reduced_sensing, 'noisy_setting': args.noisy_setting})
    else:
    ...
    #now = 1 when render
    if args.env == "trunkcube-v0":
        test_env = make_vec_env(args.env, n_envs=args.now, seed=args.seed, vec_env_cls=SubprocVecEnv, env_kwargs={'reduced_sensing': args.reduced_sensing})
    else:
        test_env = make_vec_env(args.env, n_envs=args.now, seed=args.seed, vec_env_cls=SubprocVecEnv)
